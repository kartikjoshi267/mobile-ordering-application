...
export const CartContextProvider = ({children}) => {
    ...
    const onAddToCart = async (product) => {
        try {
            const { data } = await axios.post(import.meta.env.VITE_BACKEND_URI+'/api/mobiles/cart', JSON.stringify({ _id: product._id }), {
                headers: {
    ...
    const onRemoveItem = async (productId) => {
        try {
            const { data } = await axios.delete(import.meta.env.VITE_BACKEND_URI+`/api/mobiles/cart?productId=${productId}`, {
                headers: {
                    Authorization: 'Bearer ' + getCookie('accessToken'),
                    "Content-Type": 'application/json'
                }
